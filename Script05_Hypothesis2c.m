...
clear all;
close all;
%% Hypothesis 2c: novelty effect in alpha power of posterior channels
for Subject=[1:33]
    Posterior_channels=[15:31 51:63];
    ...
    load(file_name,'Data_matrix_alpha','events','channels')
    % Separating trials in classes and performing decoding
    tr1=0;
    tr2=0;
    for trial=1:length(events)
        if strcmp(events(trial).old,'new')
            tr1=tr1+1;
            ClassA(:,:,tr1)=Data_matrix_alpha(Posterior_channels,:,trial);
        elseif strcmp(events(trial).old,'old')
            tr2=tr2+1;
            ClassB(:,:,tr2)=Data_matrix_alpha(Posterior_channels,:,trial);
        end
    end
    time_sample=0;
    for time=1:size(Data_matrix_alpha,2)
    ...
    file_name=['Z:\projects\Hamid\Projects\EEGManyPipelines\Analyses\Subject',num2str(Subject,'%02.f'),...
        '\','Subject',num2str(Subject,'%02.f'),'_Results_Hypothesis2c.mat'];
...
title ('H2c: novelty effect in Alpha band in posterior channels');