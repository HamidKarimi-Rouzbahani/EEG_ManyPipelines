...
clear all;
close all;
%% Hypothesis 1: scene category effect in voltage amplitudes of all electrodes
for Subject=[1:33]
    All_channels=[1:65];
    ...
    for trial=1:length(events)
        if strcmp(events(trial).scene_category,'manmade')
            tr1=tr1+1;
            ClassA(:,:,tr1)=Data_matrix(All_channels,:,trial);
        elseif strcmp(events(trial).scene_category,'natural')
            tr2=tr2+1;
    ...
    file_name=['Z:\projects\Hamid\Projects\EEGManyPipelines\Analyses\Subject',num2str(Subject,'%02.f'),...
        '\','Subject',num2str(Subject,'%02.f'),'_Results_Hypothesis1.mat'];
...
legend([plott.mainLine,plott.patch Sig_plot],{'Mean: natural vs. man-made';'95% CI';'Significant'},'location','northwest')
xlabel('Time relative to stimulus onset (ms)');
ylabel('Decoding accuracy (%)');
title ('H1: scene category effect in all channels');
