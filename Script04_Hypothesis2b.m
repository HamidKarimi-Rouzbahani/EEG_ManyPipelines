...
clear all;
close all;
%% Hypothesis 2b: novelty effect in theta power of frontocentral channels
for Subject=[1:33]
    ...
    load(file_name,'Data_matrix_theta','events','channels')
    ...
    for trial=1:length(events)
        if strcmp(events(trial).old,'new')
            tr1=tr1+1;
            ClassA(:,:,tr1)=Data_matrix_theta(Frontocentral_channels,:,trial);
        elseif strcmp(events(trial).old,'old')
            tr2=tr2+1;
            ClassB(:,:,tr2)=Data_matrix_theta(Frontocentral_channels,:,trial);
        end
    end
    time_sample=0;
    for time=1:size(Data_matrix_theta,2)
    ...
    file_name=['Z:\projects\Hamid\Projects\EEGManyPipelines\Analyses\Subject',num2str(Subject,'%02.f'),...
        '\','Subject',num2str(Subject,'%02.f'),'_Results_Hypothesis2b.mat'];
...
title ('H2b: novelty effect in Theta band in frontocentral channels');
